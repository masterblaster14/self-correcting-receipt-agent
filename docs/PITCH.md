...
**System.** Four layers, three of them ours.
...
2. *Perception (a pretrained vision-language model as a swappable module).* The image goes in with a strict
...
3. *Verification engine (ours, deterministic Python).* Nine constraints, no model involved. Each failure
   carries a machine-generated hypothesis: "the gap equals the CGST amount, so the paired SGST row was
   probably missed", or "the total is ten times the expected value, a decimal was dropped".
4. *Self-correction agent (ours).* Failures are ranked, their fields mapped to image zones, those zones are
...
"The perception layer is a pretrained vision-language model, which is the design in our proposal and the
standard choice in current document-AI work; LayoutLM relies on an OCR engine the same way. The perception
module is replaceable and is not the contribution. The contribution is everything that decides whether to
believe it: the constraint verifier, the hypothesis generator, the region-directed re-examination, the
...
| IV.3 OCR and spatial layout extraction | Partial | Spatial zones come from the perception model and are validated by our ink-density check; a separate OCR engine (Tesseract) is not in this version. Say: "layout is handled inside the multimodal model; a standalone OCR pass is planned as a second opinion" |
...
| V.2 Extraction layer: "Tesseract OCR and multimodal LLMs" | Partial | Multimodal model yes; Tesseract no (see IV.3) |
...
Honest tally: 17 items done, 3 partial (standalone OCR engine, AWS storage, CORD). Say the partials
yourself before anyone asks; it reads as rigour, not weakness.
